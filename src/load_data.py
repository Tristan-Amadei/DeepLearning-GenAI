...
def lire_alpha_digits(characters_to_read, datafile=datafile_alpha_digits, indices=None):
    ...
    X = np.array([datafile['dat'][indices[0]][i].flatten()
                  for i in range(len(datafile['dat'][indices[0]]))])
    for index in indices[1:]:
        X_i = np.array([datafile['dat'][index][i].flatten()
                        for i in range(len(datafile['dat'][index]))])
...
def load_mnist(train_images_filepath=train_images_filepath, train_labels_filepath=train_labels_filepath,
               test_images_filepath=test_images_filepath, test_labels_filepath=test_labels_filepath, binarize=True):
